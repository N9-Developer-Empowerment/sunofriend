...
<!-- sunofriend-interface-contract: 2026-08-18.6 -->
...
- Keep four finished-mix lanes distinct: public `broad-vocals-v1`; public
  explicit opt-in `core-four-stems-v1`; private, unregistered Mega-53 synth
  plus BS-RoFormer-SW guitar research; and the model-free recovered full-song
  listening package. The recovery package is resource-incomplete private
  evidence, not a public six-role profile or objective qualification.
- `other-refinement-v1` is an opt-in Studio challenger. It binds one exact
  ...
  noncommercial boundary. The download authority is consumed and did not by
  itself authorize installation or execution. A separately approved
  evidence-only runtime gate resolved 29 exact CPython 3.12/macOS 14+ arm64 wheels totalling
  127,527,173 bytes. Network-denied ZIP/METADATA and licence inspection ran
  ...
  That runtime authority is consumed. A later approval materialised and sealed
  the exact 144,791-byte source archive, then constructed and strictly loaded
  the 53-stem MLX model once. All 13,571 converted parameter keys, shapes and
  ...
  unregistered and non-public; three synth cases retained some missing content
  and absent-role ratings are not scored. Use
  ...
  redistribution or upload follows from the review. Reduce it with
  `scripts/record-fine-stem-downstream-midi-outcome.py`; the exact no-effects
  outcome SHA-256 is
  `d69863fb8ea59087ad6cfcd5669fee88db6afef6a7bd20809d42d88415c29a0c`.
  It retains guitar as directional private evidence and records that synth did
  not beat grouped other. The historical follow-up request was generated by
  `scripts/plan-fine-stem-synth-bottleneck.py` at SHA-256
  `d03f5ff549c5d778a6c07451c9f953be3fe29bc107e743ce5ae691e342e4419a`.
  It opens no audio. The four exact local Suno `Synth` estimates are now bound
  by `scripts/qualify-fine-stem-synth-provider-estimates.py`; the qualification
  report SHA-256 is
  `64b564798a6f338c66039e40981a5d562160c83451f2bc6d2a5411c56eba0eea`.
  All four complete provider pack sums aligned to their source windows with
  zero best envelope lag. This proves the song/clock, not the role label.
  Serve `scripts/serve-fine-stem-synth-provider-review.py`; its source/provider
  players record playback automatically and autosave without a checkbox.
  ...
  The immutable full-song plan produced by
  `scripts/plan-fine-stem-full-song-six-role.py` has exact document SHA-256
  ...
  That plan authority was approved and consumed once. The guarded execution
  retained objective failure and permits no automatic retry. The replacement
  failure retained all three model arrays, but its guitar worker result, guard
  counters and peak-memory receipt were not persisted; both failed roots stay
  immutable. A separately exact-hash-approved, network-denied recovery then
  reverified the retained PCM24, NPY and JSON identities and wrote the fixed 24
  private PCM24 review artifacts without a checkpoint load, model
  construction/load, inference, canonicalisation or model-worker subprocess.
  Its status is
  ...
  gates are incomplete and supported-ceiling compliance is unknown. The
