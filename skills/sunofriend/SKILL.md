...
- `other-refinement-v1` is an opt-in Studio challenger. It binds one exact
  ...
  It opens no audio and requires four exact provider synth/keyboard estimates
  before a separately authorised source-present three-arm MIDI experiment can
  be planned. Provider stems are comparison estimates, not truth.
