...
The original track is the primary musical input; lyrics supply words
and style modifies production. At non-zero reference strength, a result
with no owner-recognisable musical relationship to the source fails.
Provider qualification requires a matched no-reference counterfactual
so accepting an upload cannot be mistaken for using it musically.
...
A final Turbo pair improved tuning and backing musicality but retained
no audible connection to the source melody or rhythm. Turbo also cannot
validate style strength because it ignores guidance scale. ACE-Step is
rejected as the full-song remix provider for this fixture. The next gate
is a reviewed audible melody/rhythm/section scaffold before generation.
