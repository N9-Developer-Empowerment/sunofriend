...
def _render_program(
    ...
    **kwargs: object,
    ...
    amplitude = {38: 0.1, 39: 0.2}[programs[-1]]
...
class _InstrumentHTTP:
    ...
    def prepare(self) -> dict:
        lane = self.plan()["eligible_lanes"][0]
        status, payload = self.json(
            "POST",
            f"/api/instrument-review/prepare?token={self.token}",
            {
                "selection_manifest_sha256": lane[
...
def test_plan_prepare_and_private_media_are_path_free_and_fixed(
    ...
    try:
        ...
        assert len(plan["eligible_lanes"]) == 1
        assert plan["eligible_lanes"][0]["role"] == "bass"
        assert all(value is False for value in plan["effects"].values())
        assert not _contains_key(plan, "path")

        lane = plan["eligible_lanes"][0]
        invalid = {
        ...
        event_count = fixture.server.store.current_state(fixture.catalog)[
            "event_count"
        ]
        ...
        assert all(
            -60.0 <= row["applied_gain_db"] <= 0.0 for row in rows.values()
        )
        ...
        assert fixture.server.store.current_state(fixture.catalog)[
            "event_count"
        ] == event_count
        ...
        assert fixture.plan()["eligible_lanes"] == []
    finally:
        fixture.close()
...
def test_complete_and_resolve_survive_restart_and_context_eviction(
    ...
    try:
        ...
        assert {
            row["label"] for row in resolved["result"]["assignment"].values()
        } == {"Synth Bass 1", "Synth Bass 2"}
        assert resolved["result"]["resolved_choice"] == "none_usable"
        assert all(value is False for key, value in resolved["effects"].items() if key != "feedback_recorded")
