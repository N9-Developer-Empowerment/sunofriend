...
def _record(path: Path, kind: str, media_type: str) -> dict:
...
def _result(output: Path, request: dict) -> dict:
    ...
    document = {
        ...
        "checkpoint": {
            ...
            "bytes": 1_316_802_154,
            "sha256": loader.CHECKPOINT_SHA256,
        },
        "loader": loader.expected_loader_evidence(state_tensor_count=500),
        "synthetic_input": request["synthetic_input"],
        "metrics": metrics,
        "artifacts": [
            _record(first, "features_run_1", "application/x-npy"),
            _record(second, "features_run_2", "application/x-npy"),
            _record(metrics_path, "metrics", "application/json"),
        ],
        "environment": {
    ...
    _rehash(document)
    (output / "result.json").write_bytes(canonical_json_bytes(document))
    return document
...
def test_request_builder_uses_supplied_commit_without_nested_git(
    tmp_path: Path,
) -> None:
...
@pytest.mark.parametrize("change", ("network", "private", "training", "path"))
def test_request_rejects_rehashed_scope_or_authority_change(change: str) -> None:
    request = deepcopy(_request())
    if change == "network":
        request["execution"]["network_allowed"] = True
    elif change == "private":
        request["authority"]["private_audio_access_authorized"] = True
    elif change == "training":
        request["authority"]["training_execution_authorized"] = True
    else:
        request["private_path"] = "C:\\private\\song.wav"
    _rehash(request)
...
def test_result_and_round_trip_bind_exact_feature_bytes(tmp_path: Path) -> None:
    ...
    verified = verify_musicfm_synthetic_canary_round_trip(output, request)
    assert verified["status"] == "verified_synthetic_frozen_features_only"
    assert verified["private_audio_opened"] is False
...
@pytest.mark.parametrize("change", ("checkpoint", "loader", "training", "extra"))
def test_result_rejects_rehashed_false_evidence(tmp_path: Path, change: str) -> None:
...
def test_loader_migrations_are_exact_and_network_guard_restores_process() -> None:
    current = {
        "unchanged": object(),
        loader.CURRENT_WEIGHT_NORM_KEYS[0]: object(),
        loader.CURRENT_WEIGHT_NORM_KEYS[1]: object(),
    }
    legacy = {
        "unchanged": current["unchanged"],
        loader.LEGACY_WEIGHT_NORM_KEYS[0]: object(),
        loader.LEGACY_WEIGHT_NORM_KEYS[1]: object(),
    }
    migrated, evidence = loader.migrate_legacy_weight_norm_keys(legacy, current)
    assert set(migrated) == set(current)
    assert evidence == loader.WEIGHT_NORM_MIGRATIONS
    with pytest.raises(ValueError, match="key roster"):
        loader.migrate_legacy_weight_norm_keys({**legacy, "extra": object()}, current)

    original_lookup = socket.getaddrinfo
    original_cublas = os.environ.get("CUBLAS_WORKSPACE_CONFIG")
    with loader.deny_network() as attempts:
...
def test_loader_is_local_weights_only_strict_and_frozen() -> None:
...
def test_batch_norm_counter_migration_is_exact_bookkeeping_only() -> None:
    class Tensor:
        shape = ()
    ...
    state = {
        key: Tensor(Torch.float32, 95_489.0) for key in loader.BATCH_NORM_COUNTER_KEYS
    }
    expected = {key: Tensor(Torch.int64, 0.0) for key in loader.BATCH_NORM_COUNTER_KEYS}
    migrated, evidence = loader.migrate_exact_batch_norm_counters(
    ...
    changed = dict(state)
    changed[loader.BATCH_NORM_COUNTER_KEYS[0]] = Tensor(Torch.float32, 95_488.0)
    with pytest.raises(ValueError, match="BatchNorm counter changed"):
        loader.migrate_exact_batch_norm_counters(changed, expected, Torch)
