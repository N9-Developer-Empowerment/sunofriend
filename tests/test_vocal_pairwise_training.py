...
def test_cuda_request_is_exact_synthetic_offline_and_resource_bounded() -> None:
    request = build_pairwise_gpu_canary_request("a" * 40)

    assert request["experiment_id"] == GPU_EXPERIMENT_ID
    assert request["task_kind"] == "pairwise_vocal_ranker"
    assert request["method_natures"] == ["D", "T"]
    assert request["dataset"]["synthetic"] is True
    assert request["dataset"]["train_shape"] == [128, 6]
    assert request["dataset"]["heldout_shape"] == [64, 6]
    ...
    assert request["resource_ceiling"]["maximum_wall_seconds"] == 900
    ...
    assert "private_notes" not in str(request)
...
def test_cuda_result_contract_binds_full_checkpoint_roster_and_rejects_tampering() -> (
    ...
    assert {row["output_id"] for row in result["outputs"]} == {
        "metrics-json",
        "checkpoint-step-120",
        "checkpoint-final-uninterrupted",
        "checkpoint-final-resumed",
        "checkpoint-final-shuffled",
    }

    missing_checkpoint = deepcopy(result)
    missing_checkpoint["outputs"].pop()
    _rehash(missing_checkpoint)
    with pytest.raises(ValueError, match="every expected output"):
        validate_pairwise_gpu_result(missing_checkpoint, request=request)

    changed_shape = deepcopy(result)
    changed_shape["outputs"][1]["shape"]["step"] = 121
    _rehash(changed_shape)
    with pytest.raises(ValueError, match="shape changed"):
        validate_pairwise_gpu_result(changed_shape, request=request)

    leaked_path = deepcopy(result)
    leaked_path["training_evidence"]["path"] = "C:/private/checkpoint.pt"
    _rehash(leaked_path)
    with pytest.raises(ValueError, match="path"):
        validate_pairwise_gpu_result(leaked_path, request=request)

    forged_offline = deepcopy(result)
    forged_offline["environment"]["network_used"] = True
    forged_offline["environment"]["network_attempts"] = 99
    forged_offline["training_evidence"]["synthetic_only"] = False
    forged_offline["training_evidence"]["retries"] = 7
    _rehash(forged_offline)
    with pytest.raises(ValueError, match="offline|evidence"):
        validate_pairwise_gpu_result(forged_offline, request=request)
    with pytest.raises(ValueError, match="offline|synthetic"):
        validate_gpu_worker_result(forged_offline, request=request)

    forged_timing = deepcopy(result)
    forged_timing["timings"]["arms"] = 999
    forged_timing["timings"]["optimisation_steps"] = 0
