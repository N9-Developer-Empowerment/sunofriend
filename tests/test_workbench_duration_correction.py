...
_CHANGE_KEYS = {
    ...
    "before_end_tick",
    "after_end_tick",
    "before_duration_ticks",
    "after_duration_ticks",
    "tick_delta",
    "milliseconds_delta",
    "start_beat",
    "before_duration_beats",
    "after_duration_beats",
    "source_start_seconds",
    "before_source_end_seconds",
    "after_source_end_seconds",
}
_FRESH_TRUE_EFFECTS = {
    "library_mutated",
    "child_clip_created",
...
class WorkbenchDurationCorrectionTests(unittest.TestCase):
    def test_dependency_precomputation_matches_reference_and_scales_once(self) -> None:
        ...
        for _case in range(80):
            ...
            for index, note_ticks in enumerate(ticks):
                ...
                source_key = (raw.channel, raw.start_tick, raw.pitch)
                raw_group_size = sum(
                    (
                        candidate.channel,
                    ...
                    for candidate in [_raw_interval(clip, position, candidate_ticks)]
                )
                expected.append(
                    raw_group_size == 1
                    and sum(row == raw for row in normalized) == 1
                )
            self.assertEqual(
                _source_interval_exactness(clip, ticks=ticks),
                tuple(expected),
            )
        ...
        long_clip = MidiClip(
            title="Bounded long-song duration window",
            tempo_map=tempo,
            time_signature=TimeSignature(),
            instrument=Instrument("keys", 4, 0),
            notes=long_notes,
    ...
    def test_capability_exact_window_schema_four_block_reasons_and_drums(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self._services(root) as (clip_service, corrections, parent, _library):
                ...
                self.assertEqual(
                    capability["corrections"]["note_end_shift_patch"],
                    {"enabled": True, "drum_family": True},
                )
                self.assertFalse(capability["corrections"]["timing"])
                self.assertEqual(
                    capability["limits"]["maximum_note_end_delta_ticks"],
                    480,
                )
                self.assertEqual(
                    capability["limits"]["minimum_note_duration_ticks"],
                    1,
                )
                window = corrections.window(
                    self._window_request(clip_service, parent, 120, 1920)
                )
                self.assertEqual(window["schema"], CLIP_NOTE_END_WINDOW_SCHEMA)
                self.assertEqual(window["operation"], "clip-note-end-window")
                self.assertEqual(window["correction_kind"], "note_end_shift_patch")
            ...
            with self._services(
                ...
                preview = corrections.preview(
                    self._preview_request(
                        window,
                        [(editable["note_ref"], editable["end_tick"] + 30)],
                    )
                )
                self.assertEqual(preview["diff"]["changes"][0]["channel"], 9)

    def test_musical_preview_moves_only_note_ends_and_preserves_exact_contract(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with self._services(root) as (clip_service, corrections, parent, library_root):
                ...
                request = self._preview_request(
                    window,
                    [
                        (rows[0]["note_ref"], rows[0]["end_tick"] + 30),
                        (rows[1]["note_ref"], rows[1]["end_tick"] - 30),
                    ],
                )
                preview = corrections.preview(request)
                self.assertEqual(preview["schema"], CLIP_NOTE_END_PREVIEW_SCHEMA)
                self.assertEqual(preview["status"], "previewed")
                self.assertEqual(
                    preview["operation"], "clip-note-end-correction-preview"
                )
                self.assertEqual(set(preview["diff"]), _DIFF_KEYS)
                self.assertEqual(preview["diff"]["kind"], "note_end_shift_patch")
                ...
                for change in preview["diff"]["changes"]:
                    ...
                    self.assertEqual(
                        change["after_duration_ticks"]
                        - change["before_duration_ticks"],
                        change["tick_delta"],
                    )
                    self.assertEqual(
                        change["after_end_tick"] - change["start_tick"],
                        change["after_duration_ticks"],
                    )
    ...
    def test_stem_locked_positive_negative_and_signed_zero_move_only_source_end(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            with self._services(root, library_root=library_root, parent_id=parent.clip_id) as (
                ...
                request = self._preview_request(
                    window,
                    [
                        (editable[0]["note_ref"], editable[0]["end_tick"] + 30),
                        (editable[-1]["note_ref"], editable[-1]["end_tick"] - 30),
                    ],
                )
                preview = corrections.preview(request)
                ...
                changes_by_pitch = {
                    change["pitch"]: change for change in result["diff"]["changes"]
                }
                for pitch in (36, 40):
                    before = parent_by_pitch[pitch]
                    after = child_by_pitch[pitch]
                    change = changes_by_pitch[pitch]
                    self.assertEqual(
                        before.source_start_seconds,
                        after.source_start_seconds,
                    )
                    self.assertAlmostEqual(
                        after.source_end_seconds - before.source_end_seconds,
                        change["tick_delta"] * 60.0 / (120 * 480),
                    )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self._services(root) as (clip_service, corrections, parent, _library):
                ...
                with self.assertRaisesRegex(WorkbenchClipCorrectionError, "unique note refs"):
                    corrections.preview(
                        self._preview_request(
                            window,
                            [
                                (row["note_ref"], row["end_tick"] + 1),
                                (row["note_ref"], row["end_tick"] + 2),
                            ],
                        )
                    )
                context = next(
                ...
                with self.assertRaisesRegex(WorkbenchClipCorrectionError, "blocked"):
                    corrections.preview(
                        self._preview_request(
                            window,
                            [(context["note_ref"], context["end_tick"] + 1)],
                        )
                    )
            ...
            with self._services(
                ...
                preview = corrections.preview(
                    self._preview_request(
                        window,
                        [(row["note_ref"], row["end_tick"] + 1) for row in rows[:64]],
                    )
                )
    ...
    def test_lifetime_dependency_target_overlap_and_each_horizon_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self._services(root) as (clip_service, corrections, parent, _library):
                ...
                later_same_pitch = next(
                    row
                    for row in window["notes"]
                    if row["pitch"] == 64 and row["start_tick"] == 960
                )
                self.assertTrue(later_same_pitch["editable"])
                for row in lifetime:
                    with self.assertRaisesRegex(WorkbenchClipCorrectionError, "blocked"):
                        corrections.preview(
                            self._preview_request(
            ...
            with self._services(
                ...
                with self.assertRaisesRegex(WorkbenchClipCorrectionError, "overlap"):
                    corrections.preview(
                        self._preview_request(window, [(first["note_ref"], 961)])
                    )
    ...
    def test_create_replay_restart_exact_delta_forgery_and_deterministic_midi(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self._services(root) as (clip_service, corrections, parent, library_root):
                ...
                row = next(item for item in window["notes"] if item["editable"])
                request = self._preview_request(
                    window, [(row["note_ref"], row["end_tick"] + 30)]
                )
                preview = corrections.preview(request)
                result = corrections.create(self._create_request(preview, request))
                self.assertEqual(result["schema"], CLIP_NOTE_END_RESULT_SCHEMA)
                ...
                self.assertEqual(child.transform_recipe.operation, "shift_note_ends")
                ...
                replacement = next(
                    ...
                    if interval.start_tick == change["start_tick"]
                ...
                forged = replace(
                    ...
                    transform_recipe=TransformRecipe.create(
                        "shift_note_ends", **forged_parameters
                    ),
                )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self._services(root / "first", library_root=library_root, parent_id=parent.clip_id) as (
                ...
                first_request = self._preview_request(
                    first_window, [(moving["note_ref"], moving["end_tick"] + 480)]
                )
                ...
                identical_request = self._preview_request(
                    second_window,
                    [(second_moving["note_ref"], second_moving["end_tick"] + 480)],
                )
                identical_preview = second.preview(identical_request)
                # Extending past a different-pitch note is legal and ordering remains canonical.
            ...
            with self._services(
                ...
                areq = self._preview_request(aw, [(ar["note_ref"], ar["end_tick"] + 30)])
                breq = self._preview_request(bw, [(br["note_ref"], br["end_tick"] + 60)])
                ap = a.preview(areq)
                bp = b.preview(breq)
                ...
                for name, mutate in (
                    ...
                    (
                        ...
                        lambda value: value["correction"]["changes"][0].update(
                            target_end_tick=row["end_tick"] + 60
                        ),
                    ),
                    (
                        "signed-zero-after",
    ...
    def _parent() -> MidiClip:
        ...
        return MidiClip(
            ...
            provenance=Provenance(
                source_uri="/Users/private/duration.wav",
                source_stem="/Users/private/duration.wav",
                converter="test",
                details={"timing_mode": "musical"},
            ),
            clip_id="duration-parent",
        )
    ...
    def _overlap_parent() -> MidiClip:
        ...
        return MidiClip(
            ...
            clip_id="overlap-duration-parent",
        )
    ...
    def _horizon_parent(kind: str) -> MidiClip:
        ...
        if kind == "beat":
            ...
            note = replace(note, source_start_seconds=0.5, source_end_seconds=2.5)
            chords = (ChordEvent(0, 4, "C", 0.0, 2.0),)
        else:
            raise AssertionError(kind)
        return MidiClip(
            title=f"{kind} horizon duration parent",
            tempo_map=tempo,
            time_signature=TimeSignature(),
            instrument=Instrument("lead", 80, 0),
            notes=(note,),
            chords=chords,
            clip_id=f"{kind}-horizon-duration-parent",
        )

    @staticmethod
    def _crossing_parent(*, clip_id: str = "crossing-duration-parent") -> MidiClip:
        tempo = TempoMap.constant(120)
        return MidiClip(
            title="Crossing duration parent",
            tempo_map=tempo,
            time_signature=TimeSignature(),
            instrument=Instrument("keys", 0, 0),
            notes=(
    ...
    def _preview_request(window: dict, changes: list[tuple[str, object]]) -> dict:
        return {
            ...
            "correction": {
                ...
                "changes": [
                    {"note_ref": note_ref, "target_end_tick": target}
    ...
    def _assert_path_free(root: Path, *values: object) -> None:
        ...
        if str(root) in encoded or "/Users/private" in encoded:
            raise AssertionError("public duration response contains a local path")
