...
def test_query_runtime_audit_names_hidden_artifact_and_forbids_loaders() -> None:
    ...
    assert audit["status"] == "blocked_pending_passt_checkpoint_evidence"
    ...
    assert passt["sha256"] is None
    assert passt["download_approved"] is False
    ...
    assert audit["proposed_runtime_identity"]["artifact_hashes_complete"] is False
