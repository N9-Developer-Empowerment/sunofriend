...
(() => {
  ...
  function qualityHtml(name) {
    const records = [
      ...overlayRecords("risk", name).map(record => `<div class="dependency">${badgeHtml(`CRAP ${record.crap_score ?? "unmeasured"}`, record.status === "warning" ? "bad" : record.status === "ok" ? "good" : "")}${badgeHtml(record.attachment, record.attachment === "current" ? "good" : "bad")}<small>${escapeText(record.qualified_name)} · complexity ${record.complexity} · coverage ${record.coverage_percentage ?? "unmeasured"}% · line ${record.line}</small></div>`),
      ...
      ...overlayRecords("mutation", name).map(record => `<div class="dependency">${badgeHtml(record.status, record.status === "survived" ? "bad" : record.status === "killed" ? "good" : "")}${badgeHtml(record.attachment, record.attachment === "current" ? "good" : "bad")}${badgeHtml(`run ${record.document.run_status}`)}<small>${escapeText(record.qualified_name || "module")}: ${escapeText(record.operator)} at line ${record.line}. Equivalence remains a separate review dimension.</small></div>`),
    ];
