...
Increments 6.0, 6.1, 6.2a and 6.3a–e are complete while broader Phase 6
remains in progress.
...
only for explicit hybrid construction. They do not block Clip browsing,
audition, reconstruction, explicit reuse proposals, reviewed key/BPM children
or the bounded user-authored correction slices. Conditional medium/large
checkpoint and other-hardware measurements are not Phase 6 entry gates.
...
explicit main/optional MIDI lane, and supplies only four canonical groups.
The browser sends its hash plus a 0.5–15 second window—not track IDs, roles,
gains or a chosen process. The server rechecks current state before and after
rendering, registers no stale media after a concurrent decision, uses current
