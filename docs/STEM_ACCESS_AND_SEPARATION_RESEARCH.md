...
| --- | --- | --- | --- | --- |
| [Demucs / HTDemucs](https://github.com/facebookresearch/demucs) | Stable four-source vocabulary: drums, bass, vocals, other. Experimental `htdemucs_6s` adds guitar and piano, but the official README warns that piano has substantial bleed and artefacts. | The Apple-native [Demucs-MLX](https://github.com/ssmall256/demucs-mlx) six-source conversion is the installed `other-refinement-v1` Studio challenger. Its explicit local safetensors/config route keeps named resolution and first-run conversion disabled. The one exact in-memory `Fraction(39, 5)` normalization passed under network denial without mutating the config, followed by a synthetic canary and both advertised target mappings on an authorised 234-second parent. | The runtime and model repository declare MIT metadata; the 109,726,583-byte checkpoint, 1,946-byte config, source revisions and SHA-256s have immutable profile entries. No bespoke permission letter is treated as an indefinite local-preview blocker unless contradictory evidence appears. | The completed fixed five-song, ten-report round demonstrated neither useful guitar extraction nor successful piano extraction. The technically valid profile remains reproducible in Studio but is not promoted or selected for MIDI. |
| [Banquet / Query Bandit](https://github.com/kwatcharasupat/query-bandit) | Query-conditioned music separation with published training classes for acoustic/clean/distorted guitar, electric piano, organ, synth pad and synth lead. It requires one ten-second audio query rather than a fixed decoder role. | Source revision `79ed5bb75e5c3a40cd319d9d990cee913fc65c26` has an exact isolated CPython 3.12.10/macOS-arm64 runtime, completed network-denied construction/load evidence and one successful generated-tensor synthetic forward. A later separately approved rights-bound canary completed exactly nine song-disjoint CPU attempts in 64.01 seconds at 3.26 GB peak RSS. All 36 private PCM24 artifacts matched their hashes and reconstructed within zero LSB, with zero network or forbidden-audio attempts. Its completed review rated eight targets not useful and one quiet keyboard target partly useful. The proposed profile remains unregistered and non-executable. | MIT source. The Banquet checkpoint SHA-256 is `657295888781e62ef50593002720d2edb3858b9e5bbfabf0c54f715a0da4b9e2` under CC BY-NC-SA 4.0. The required OpenMIC PaSST SHA-256 is `dc229428753176e8be0373d25887116fc15b490af86f671cecf9ed76a0f287da`. The restricted gate matched all 1,228 combined state keys, shapes and dtypes and strictly loaded both with zero network or audio access. | First bounded candidate for guitar plus broad `keyboard_synth`; technically valid but musically unsuccessful in the frozen canary. Local noncommercial research only; no hosting, checkpoint redistribution, commercial default, automatic source choice or MIDI activation. Both approved inference authorities are consumed; poor feedback cannot trigger a query hunt, and further inference needs a new reviewed plan and explicit approval. |
...
- [ ] Remove or explicitly confine extension/runtime path TOCTOU, and bind the
  ...
  also bound to one exact authorised worker run: the python.org launcher
  transitioned to a distinct signed `Python.app` image whose kernel CDHash
  matched the parent's strict static identity. The exact `sandbox-exec`
...
- [x] Complete the one-unit blind primary-versus-lowest review for the second
  song and resolve its answer key only after the matching export. After the
  listener understood the male-lead focus, the corrected blind export
  preferred the 16-note lowest-register hypothesis over the unchanged 23-note
  primary contour. Retain it as an audition option; no singer-identity rule,
  default or promotion changed.
- [x] Repeat the separate-vocal-leaf observation on two disjoint songs. `I am
  a Alien mashup` retained non-empty broad MIDI and all six leaves produced at
  least one non-empty primary; `Be Alone` retained non-empty broad MIDI and
  four of six leaves produced candidates. Together with the `Pilé` broad-sum
...
- [x] Add an inactive phrase-time completeness diagnostic that gives each
  ...
  vocal activity. On `I am a Alien mashup`, the primary covers 4.473693 of
  ...
  default.
- [x] Repeat phrase-time completeness song-disjoint on `Be Alone`, regenerating
  only downstream MIDI and leaf evidence from the already sealed Kim output.
  The primary covers 3.360474 of 5.980367 consensus seconds; the two-note
