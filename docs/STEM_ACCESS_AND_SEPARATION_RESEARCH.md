...
| --- | --- | --- | --- | --- |
| [Demucs / HTDemucs](https://github.com/facebookresearch/demucs) | Stable four-source vocabulary: drums, bass, vocals, other. Experimental `htdemucs_6s` adds guitar and piano, but the official README warns that piano has substantial bleed and artefacts. | The Apple-native [Demucs-MLX](https://github.com/ssmall256/demucs-mlx) six-source conversion is now the first pinned `other-refinement-v1` Studio challenger. Its explicit local safetensors/config route must keep named resolution and first-run conversion disabled. The config repeats the known `"39/5"` segment string, so the candidate remains blocked pending one exact in-memory fraction remediation under network denial. | The runtime and model repository declare MIT metadata; the 109,726,583-byte checkpoint, 1,946-byte config, source revisions and SHA-256s now have an explicit immutable profile entry. No bespoke permission letter is treated as an indefinite local-preview blocker unless contradictory evidence appears. | Exact approval-gated Studio plan for direct experimental guitar and a disclosed piano-as-keys proxy; uninstalled, non-executable and unable to delay or disable public core four. |
