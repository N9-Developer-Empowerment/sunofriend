"""Stable receipt contract for one bounded browser microphone capture.

This module owns path-free capture validation without importing the Musical
State facade. Callers must supply an already validated Musical State document.
"""
...
def validate_vocal_capture_against_state(
    capture: Mapping[str, Any], state: Mapping[str, Any]
) -> dict[str, Any]:
    """Validate a bounded capture against an already validated Musical State."""
...
def _validate_capture_binding_and_identity(
    ...
    if document.get("binding") != {
        "musical_state_schema": state["schema"],
