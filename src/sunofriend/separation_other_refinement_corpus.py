...
<button type="button" class="download-one">Download this review JSON</button><button type="button" class="copy-one secondary">Copy text-only feedback</button><span class="message" aria-live="polite"></span><textarea class="manual-output" aria-label="Review JSON or text backup" readonly hidden></textarea>
</section>'''
        )
    bindings = [
        {
...
:root{{--ink:#f4f7fb;--muted:#b9c6d5;--night:#071321;--panel:#102031;--line:#31506a;--cyan:#4fe2ee}}*{{box-sizing:border-box}}body{{margin:0;background:var(--night);color:var(--ink);font:17px/1.5 system-ui,sans-serif}}main{{max-width:1240px;margin:auto;padding:30px 20px 90px}}h1{{font-size:clamp(2.4rem,7vw,5rem);line-height:1;margin:.2em 0}}.lede{{max-width:900px;font-size:1.15rem}}.notice,.case{{border:1px solid var(--line);border-radius:18px;background:var(--panel);padding:22px;margin:20px 0}}.notice{{border-left:6px solid #ffd666}}.eyebrow{{color:var(--cyan);font-weight:800;letter-spacing:.12em;text-transform:uppercase}}.audio-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}}.audio-card{{background:#091522;border:1px solid var(--line);border-radius:12px;padding:13px}}.audio-card h3{{margin:.1em 0}}audio{{width:100%}}.small,.message{{color:var(--muted);font-size:.85rem}}label{{display:block;margin:12px 0;font-weight:650}}.check{{padding:14px;background:#152a3d;border-radius:10px}}input{{width:1.15rem;height:1.15rem;vertical-align:-.15rem}}select,textarea{{display:block;width:100%;margin-top:5px;padding:10px;background:#071321;color:var(--ink);border:1px solid var(--line);border-radius:8px;font:inherit}}textarea{{min-height:90px}}.manual-output{{min-height:120px;font:13px/1.35 ui-monospace,SFMono-Regular,monospace}}.manual-output[hidden]{{display:none}}.fields{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}}button{{border:0;border-radius:9px;padding:12px 16px;background:var(--cyan);font:inherit;font-weight:800;margin:5px;cursor:pointer}}button.secondary{{background:#244966;color:white}}.sticky{{position:sticky;bottom:0;background:#071321ee;border-top:1px solid var(--line);padding:14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}}@media(max-width:800px){{.audio-grid,.fields{{grid-template-columns:1fr}}}}
</style></head><body><main><div class="eyebrow">Sunofriend · bounded local evaluation</div><h1>Five songs. Two targets. No automatic winner.</h1><p class="lede">Review exactly ten frozen cases. Moises and Suno stems are independent comparison estimates, never ground truth. The keys target is the model's piano proxy, not general keyboards or synths. Poor or mixed feedback remains evidence and does not disable the installed challenger.</p><div class="notice"><b>Privacy:</b> this localhost page makes no network requests and accepts no uploads. Review JSON contains result hashes and answers, but no audio, filenames or browser telemetry.</div>{"".join(cards)}<div class="sticky"><button id="download-all" type="button">Download all completed reviews</button><button id="copy-summary" class="secondary" type="button">Copy text-only summary</button><strong id="progress">0 / {len(bindings)} listened</strong><span id="global-message" class="message" aria-live="polite"></span><textarea id="global-output" class="manual-output" aria-label="Ten-review JSON or text backup" readonly hidden></textarea></div></main><script>
const bindings={bindings_json};const indexHash='{index["document_sha256"]}';const sections=[...document.querySelectorAll('[data-case]')];const globalMessage=document.getElementById('global-message');const globalOutput=document.getElementById('global-output');const val=(section,selector)=>section.querySelector(selector).value;function review(section,binding){{return{{schema:'sunofriend.other-refinement-listening.v1',result_sha256:binding.result_sha256,target_id:binding.target_id,listened:section.querySelector('.listened').checked,usefulness:val(section,'.usefulness'),bleed:val(section,'.bleed'),missing_content:val(section,'.missing'),artefacts:val(section,'.artefacts'),timing_or_join_problems:val(section,'.timing'),downstream_midi:val(section,'.midi'),notes:val(section,'.notes'),activation_choice:'none',exported_at:new Date().toISOString()}}}}function showOutput(output,text){{output.hidden=false;output.value=text}}function saveJson(value,name,output,message){{const text=JSON.stringify(value,null,2)+'\\n';showOutput(output,text);const blob=new Blob([text],{{type:'application/json'}});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);message.textContent='Download requested; a local text backup is shown below.'}}async function copyText(text,output,message){{showOutput(output,text);try{{await navigator.clipboard.writeText(text);message.textContent='Text copied and shown below.'}}catch(error){{output.focus();output.select();message.textContent='Clipboard unavailable; copy the selected text below.'}}}}function update(){{const count=sections.filter(section=>section.querySelector('.listened').checked).length;document.getElementById('progress').textContent=count+' / '+bindings.length+' listened'}}sections.forEach((section,index)=>{{section.addEventListener('change',update);const message=section.querySelector('.message');const output=section.querySelector('.manual-output');section.querySelector('.download-one').onclick=()=>{{const data=review(section,bindings[index]);saveJson(data,'sunofriend-other-refinement-listening-'+bindings[index].case_id+'.json',output,message)}};section.querySelector('.copy-one').onclick=async()=>{{const d=review(section,bindings[index]);const text=`Sunofriend ${{bindings[index].target_id}} review: usefulness=${{d.usefulness}}; bleed=${{d.bleed}}; missing=${{d.missing_content}}; artefacts=${{d.artefacts}}; timing=${{d.timing_or_join_problems}}; MIDI=${{d.downstream_midi}}; notes=${{d.notes||'none'}}`;await copyText(text,output,message)}}}});document.getElementById('download-all').onclick=()=>{{const incomplete=sections.filter(section=>!section.querySelector('.listened').checked);if(incomplete.length){{globalMessage.textContent='Listen to all ten cases before downloading the bounded bundle.';incomplete[0].scrollIntoView({{behavior:'smooth'}});return}}const data={{schema:'{CORPUS_LISTENING_SCHEMA}',review_index_sha256:indexHash,reviews:sections.map((section,index)=>({{case_id:bindings[index].case_id,review:review(section,bindings[index])}})),audio_included:false,filenames_included:false,browser_telemetry_included:false,exported_at:new Date().toISOString()}};saveJson(data,'sunofriend-other-refinement-corpus-listening.json',globalOutput,globalMessage)}};document.getElementById('copy-summary').onclick=async()=>{{const lines=sections.map((section,index)=>{{const d=review(section,bindings[index]);return `${{index+1}}. ${{bindings[index].target_id}}: ${{d.usefulness}}; bleed=${{d.bleed}}; missing=${{d.missing_content}}; artefacts=${{d.artefacts}}; timing=${{d.timing_or_join_problems}}; MIDI=${{d.downstream_midi}}; notes=${{d.notes||'none'}}`}});await copyText(lines.join('\\n'),globalOutput,globalMessage)}};update();
</script></body></html>"""
