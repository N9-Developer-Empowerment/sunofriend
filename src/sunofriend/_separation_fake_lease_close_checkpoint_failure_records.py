"""Pure evidence for checkpoint mutation during final lease close.

This boundary is intentionally disjoint from the immediate post-core and FD5
reservation-release checkpoint failure receipts.  It describes a fixed worker
that completed and was exactly reaped, successful parent post-core and
reservation-release checkpoint remeasurements, normal bridge finish and FD5
release, followed by a final checkpoint-lease-close remeasurement that found
one admitted identity, byte-count or hash change.  The checkpoint descriptor
must still have closed cleanly.

The receipt is historical and inert.  A final-close mismatch does not prove
which checkpoint bytes the child executed, whether any checkpoint bytes were
deserialized, or when the observed mutation occurred.
"""
...
_SCHEMA = "sunofriend.separation-fake-lease-close-checkpoint-failure.v1"
_POLICY_ID = "private-lease-close-checkpoint-integrity-failure-v1"
_PRIMARY_SCOPE = "parent_checkpoint_lease_close_integrity"
_PRIMARY_STAGE = "checkpoint_lease_close_checkpoint_remeasurement"
...
class _SeparationFakeExecutionLeaseCloseCheckpointFailedReceipt(
    Mapping[str, Any]
):
    """Immutable, path-free final lease-close integrity evidence."""
...
def _build_lease_close_checkpoint_failed_terminal_receipt(
    *,
...
) -> _SeparationFakeExecutionLeaseCloseCheckpointFailedReceipt:
    """Seal one exact, inert final lease-close integrity failure."""
    ...
    if (
        ...
        raise ValueError("lease-close native success binding changed")
    ...
    if any(
        ...
        raise ValueError("lease-close checkpoint lease binding changed")
    ...
    payload = {
        ...
        "status": (
            ...
            if cleanup
            else "failed_lease_close_checkpoint_integrity"
        ),
    ...
    return _validate_lease_close_checkpoint_failed_terminal_receipt(
        _wrapper(
            _freeze(
                {
                    **payload,
...
def _validate_lease_close_checkpoint_failed_terminal_receipt(
    value: Any,
) -> _SeparationFakeExecutionLeaseCloseCheckpointFailedReceipt:
    """Validate one exact final lease-close integrity receipt."""

    if type(value) is not _SeparationFakeExecutionLeaseCloseCheckpointFailedReceipt:
        raise ValueError("lease-close checkpoint failure receipt type is invalid")
    document = _plain(value)
    if not isinstance(document, dict) or set(document) != _FIELDS:
        raise ValueError("lease-close checkpoint failure fields are invalid")
    _validate_path_free(document, "lease-close checkpoint failure receipt")
    if (
        ...
        raise ValueError("lease-close checkpoint failure policy is invalid")
    ...
    if (
        ...
        raise ValueError("lease-close checkpoint bindings are invalid")
    ...
    if not isinstance(failure, dict) or set(failure) != {
        ...
        raise ValueError("lease-close checkpoint failure evidence is invalid")
    ...
    if (
        ...
        raise ValueError("lease-close checkpoint primary evidence is invalid")
    ...
    if (
        ...
        raise ValueError("lease-close checkpoint cleanup evidence is invalid")
    for ordinal, event in enumerate(cleanup):
        if (
            ...
            raise ValueError(
                "lease-close checkpoint cleanup evidence is invalid"
            )
    expected_status = (
        "failed_lease_close_checkpoint_integrity_with_cleanup_failures"
        if cleanup
    ...
    if document["status"] != expected_status:
        raise ValueError(
            "lease-close checkpoint failure status is inconsistent"
        )
    if not _valid_process(document["process"]):
        raise ValueError("lease-close checkpoint process evidence is invalid")
    if document["checkpoint"] != _CHECKPOINT:
        raise ValueError("lease-close checkpoint proof evidence is invalid")
    ...
    if (
        ...
        raise ValueError("lease-close checkpoint lease evidence is invalid")
    if document["outputs"] != _OUTPUTS:
        raise ValueError("lease-close checkpoint output evidence is invalid")
    if document["permissions"] != _PERMISSIONS:
        raise ValueError("lease-close checkpoint permissions are invalid")
    if document["limitations"] != _LIMITATIONS:
        raise ValueError("lease-close checkpoint limitations are invalid")
    ...
    if (
        ...
        raise ValueError("lease-close checkpoint self-hash is invalid")
    return value
...
def _validate_failed_lease_terminal_receipt(
    ...
    if type(value) is not SeparationCheckpointDescriptorLeaseTerminalReceipt:
        raise ValueError(
            "lease-close checkpoint failure requires an exact lease receipt"
        )
    document = _plain(value)
    try:
        ...
        raise ValueError(
            "lease-close checkpoint lease receipt is invalid"
        ) from exc
    if (
        ...
        raise ValueError(
            "lease-close checkpoint lease receipt is not a narrow integrity "
            "failure with complete cleanup"
        )
    return document
