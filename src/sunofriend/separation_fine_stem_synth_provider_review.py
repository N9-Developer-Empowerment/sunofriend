    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if route in {"/", "/REVIEW/provider_review.html"}:
                ...
                self.send_header(
                    "Content-Disposition",
                    'attachment; filename="sunofriend-provider-synth-presence-review.json"',
                )
        ...
        def do_HEAD(self) -> None:  # noqa: N802
            route = self.path.partition("?")[0]
            if route in routes:
                self._file(routes[route], body=False)
            else:
                self.send_error(404)
        ...
        def _file(self, path: Path, *, body: bool = True) -> None:
