"""Non-importing evidence for the query challenger's runtime wheel closure."""
...
RUNTIME_WHEEL_EVIDENCE_SCHEMA = (
    "sunofriend.other-refinement-query-runtime-wheel-evidence.v1"
)
...
def canonicalize_distribution_name(value: str) -> str:
    """Return the package-name normalization used by Python repositories."""
...
def inspect_runtime_wheel_evidence(wheel_directory: str | Path) -> dict[str, Any]:
    """Hash wheels and inspect only their ZIP metadata and licence records."""
        ...
        try:
            with zipfile.ZipFile(wheel) as archive:
                ...
                licence_members = sorted(
                    ...
                    or Path(name).name.lower().startswith(("license", "licence", "copying"))
                )
                licence_evidence = []
