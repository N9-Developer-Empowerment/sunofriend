...
experimental two-stem default, explicit SCNet core-four opt-in, existing stems, help obtaining stems or the
...
discovery. The feature branch now also has a tested native source-lineage lock,
and one `listen` conversion/publication path passes; complete demo/create,
Studio and TUI journeys remain unqualified. The public `/windows/` guide
records the exact setup and boundary without claiming Windows support. Linux
and Windows Subsystem for Linux remain unverified. The site asks people to
share follow-up results through the compatibility form so SKILL.md and setup
can improve.
...
- `/windows/` records the partially verified native-Windows setup, diagnostic
  commands, fixed locking blocker and remaining verification boundary;
...
instrumental on supported Apple-silicon Macs; an explicitly installed SCNet
profile can instead estimate vocals, drums, bass and grouped other. Synth and
guitar six-role work remains private, unregistered research, and separation
does not start MIDI conversion automatically.
